# =============================================================================
# trace — Alpine container image (Unraid-compatible)
# =============================================================================
...
#     DATA_DIR env var if needed.
#   - Listens on 0.0.0.0:<port> for bridge networking.
#
# Build (Podman):
#   cd ~/projects/studio2201/trace
#   podman build -f Containerfile.ubi -t trace:latest .
#
# Run (Unraid-style):
#   podman run -d --name trace \
#     -p <host-port>:4404 \
#     -v /mnt/user/appdata/trace:/config \
#     -e TRACE_PIN=<your-pin> \
#     trace:latest
#

# Compare size:
#   podman images | grep -E 'trace|studio2201/trace'
# =============================================================================
...
LABEL org.opencontainers.image.title="trace" \
      org.opencontainers.image.description="trace (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/trace" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/trace
RUN chmod +x /usr/local/bin/trace
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4404}/health" >/dev/null || exit 1
